...
온라인 리테일 거래 데이터를 기반으로 고객 가치를 세분화하고, 실제 마케팅 예산을 어디에 집중해야 하는지 정리한 CRM 분석 프로젝트입니다. 핵심은 `고객 분류` 자체보다 **누구에게 먼저 액션해야 하는가**를 데이터로 설명한 점입니다.

이 프로젝트는 Starbucks Promotion Analysis와 역할이 다릅니다. Starbucks가 고객-오퍼 반응 예측과 캠페인 후보 추천에 가깝다면, UK Online Retail은 RFM 기반 고객 세그먼트를 **유지·재활성화·빈도 상승 액션**으로 번역한 프로젝트입니다.

## 3-Minute Reviewer Path

| Step | Open | What to check |
| --- | --- | --- |
| 1 | [한 페이지 요약](./docs/한페이지_요약.md) | 문제 정의, 핵심 결과, 실행 시나리오 |
| 2 | [발표 자료 가이드](./docs/PPT_Retail.md) | Pareto, 통계 검정, 세그먼트 해석 근거 |
| 3 | [RFM segment overview](./리테일_시각화_png/도넛차트.png) | 고객군 분포와 세그먼트 구조 |
| 4 | [Monthly sales trend](./리테일_시각화_png/월별고객별매출추이&성장률분석.png) | 월별 매출 변화와 이탈위험 고객군 신호 |
| 5 | [VERIFY.md](./VERIFY.md) | 공개 검증 범위, CI 명령, 원본 데이터 경계 |

## Decision Design Snapshot

| Design Point | Why it matters |
| --- | --- |
| **Monetary separation** | RFM을 그대로 쓰지 않고 `Monetary`를 결과 해석 변수로 분리해 순환 논리를 줄임 |
| **Segment validity** | 파레토 분석과 ANOVA/Kruskal-Wallis를 함께 사용해 세그먼트 차이가 우연인지 점검 |
| **Action translation** | 고객군 분류를 유지, 재활성화, 구매 빈도 상승, 저비용 nurture 액션으로 연결 |
| **CRM positioning** | 고객군 이름보다 예산·채널·KPI를 함께 제안해 마케팅 실행 기준으로 번역 |
| **Public repository cleanup** | 노트북 중심 결과물을 README, docs, `run_pipeline.py` 기준으로 다시 검토 가능하게 정리 |
...
이 저장소는 4인 팀 프로젝트 결과를 공개 저장소 기준으로 다시 정리한 버전입니다. 개별 기여를 과장하기보다, 이 프로젝트에서 실제로 읽히길 원하는 contribution signal을 아래 네 가지에 맞춰 정리했습니다.
